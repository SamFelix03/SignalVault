...
import { cn, formatPoolPrice, formatPnlPercent, timeAgo } from '@/lib/utils'
            ...
            {trades.map((trade, i) => (
              <TableRow key={`${trade.epoch}-${i}`} className="group">
                ...
                <TableCell className="text-right font-mono text-sm">
                  ${formatPoolPrice(trade.entryPrice)}
                </TableCell>
                <TableCell className="text-right font-mono text-sm">
                  ${formatPoolPrice(trade.exitPrice)}
                </TableCell>
                ...
                <TableCell className="text-center">
                  {trade.reasoningHash ? (
                    <Link
                      href={(trade.vaultAddress ?? vaultAddress) ? `/vault/${trade.vaultAddress ?? vaultAddress}/audit/${trade.reasoningHash}` : '#'}
