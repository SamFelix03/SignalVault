...
/// @notice PerpRouter v5: Pull margin AFTER successful IOC fill to prevent follower loss on thin books.
/// Key change from v4: margin is only moved if an order actually gets filled.
contract PerpRouterV5 is IExecutionRouter {
  /// @dev Version 5: margin-pull-after-fill to prevent losses on IOC no-fill.
  uint256 public constant ROUTER_VERSION = 5;

  /// @dev Floor slippage for IOC mirrors on thin testnet books.
  uint16 internal constant MIN_MIRROR_SLIPPAGE_BPS = 200;
  /// @dev Max slippage attempted across the retry ladder.
  uint16 internal constant MAX_MIRROR_SLIPPAGE_BPS = 20000; // EXTREME testnet slippage (200%)
  ...
  /// @notice CREATE2 address of the follower's mirror wallet.
  ...
  function placeOrder(
    ...
    // Try IOC at each slippage level WITHOUT funding margin first
    for (uint8 attempt = 0; attempt < MAX_FILL_ATTEMPTS; attempt++) {
      ...
      // Test if order would fill at this price/quantity WITHOUT pulling margin
      try IFollowerMirrorWallet(wallet).executeOpen(pool, 0, isBid, price, quantity, expireNs) returns (
        uint128 orderId
      ) {
        if (orderId != 0) {
          // Order succeeded with no margin - that means it would have worked
          // But we need to fund margin for the actual execution
          // So now fund margin and do it for real
          try IFollowerMirrorWallet(wallet).fundMargin(sizeAmount) {
            // Margin funded, try the order again
            try IFollowerMirrorWallet(wallet).executeOpen(pool, 0, isBid, price, quantity, expireNs) returns (
              uint128 realOrderId
            ) {
              if (realOrderId != 0) {
                emit OrderMirrored(follower, wallet, pool, direction, sizeAmount, quantity, realOrderId);
                return bytes32(uint256(realOrderId));
              }
            } catch {
              // Order failed after funding - unusual but possible on fast-moving books
            }
          } catch {
            // Funding failed - insufficient allowance or balance
          }
        }
      } catch {
        // Order would fail even with margin, try next slippage
      }
  ...
  /// @dev IOC mirrors always price from live mark + slippage.
